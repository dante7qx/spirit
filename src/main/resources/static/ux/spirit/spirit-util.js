...
spirit.util = {
		isEditForm: function (id, flag, spCompsIds) {
        	if(spCompsIds!=null && spCompsIds.length>0) {
        		$.each(spCompsIds, function(key, val) {
        			spirit.util.isEditFormCommon(id,flag,val);
        		});
        	} else {
        		spirit.util.isEditFormCommon(id,flag,null);
        	}
		},
		isEditFormCommon : function(id, flag, spId){
	        ...
	        if (flag) {   
	            ...
	            if(partEditFlag){
	            	...
	            	$('.numberbox,.textbox', '#' + id).removeClass('formborder');
	            }
	        } else {
	            $('input[type="text"]'+val+',textarea'+val+',input[type="file"]'+val+'', '#' + id).each(function(){
	            ...
	            if(partEditFlag){//部分不可编辑
	            	...
	            	$('.numberbox,.textbox', '#' + id).addClass('formborder');
	            }
	        }
	    },
	    ...
	    tableMerges : function (id, fieldArr, befField) {
			for ( var i = 0; i < fieldArr.length; i++) {
				if(befField) {
					spirit.util.tableMergeBef(id, fieldArr[i], befField);
				} else {
					spirit.util.tableMerge(id, fieldArr[i]);
				}
			}
		},
		...
		tableMergeBef : function(id, field, befField) {
			...
			for ( var i = 0; i < rows.length; i++) {
				var row = rows[i];
				var realVal = row[field] + '_' + (row[befField]? row[befField] : spirit.util.guidGenerator());
				if (i == 0) {
					rowStr = realVal;
					var merObj = {};
					merObj.index = 0;
